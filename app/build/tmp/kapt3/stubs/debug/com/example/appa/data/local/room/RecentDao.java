package com.example.appa.data.local.room;

/**
 * Data Access Object (DAO) interface for accessing recent items data in the database.
 * @author AnhTTH
 */
@kotlin.Metadata(mv = {1, 9, 0}, k = 1, xi = 48, d1 = {"\u0000*\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010 \n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\t\n\u0002\b\u0002\n\u0002\u0010\u0002\n\u0000\n\u0002\u0010\u000e\n\u0002\b\u0004\bg\u0018\u00002\u00020\u0001J\u000e\u0010\u0002\u001a\b\u0012\u0004\u0012\u00020\u00040\u0003H\'J\u0010\u0010\u0005\u001a\u00020\u00062\u0006\u0010\u0007\u001a\u00020\u0004H\'J\u0018\u0010\b\u001a\u00020\t2\u0006\u0010\n\u001a\u00020\u000b2\u0006\u0010\f\u001a\u00020\u000bH\'J\u0018\u0010\r\u001a\u00020\t2\u0006\u0010\u000e\u001a\u00020\u00062\u0006\u0010\f\u001a\u00020\u000bH\'\u00a8\u0006\u000f"}, d2 = {"Lcom/example/appa/data/local/room/RecentDao;", "", "getALlRecent", "", "Lcom/example/appa/data/model/ItemRecent;", "insertRecent", "", "imageDetail", "updatePathDownload", "", "pathDownload", "", "path", "updateTimeRecent", "time", "app_debug"})
@androidx.room.Dao()
public abstract interface RecentDao {
    
    /**
     * Retrieves all recent items from the database ordered by their ID in descending order.
     * @return a list of all recent items.
     * @author AnhTTH
     */
    @androidx.room.Query(value = "SELECT * FROM ItemRecent ORDER BY id DESC")
    @org.jetbrains.annotations.NotNull()
    public abstract java.util.List<com.example.appa.data.model.ItemRecent> getALlRecent();
    
    /**
     * Updates the time added of a recent item in the database.
     * @param time the new time to be set.
     * @param path the path of the recent item to be updated.
     * @author AnhTTH
     */
    @androidx.room.Query(value = "update ItemRecent set timeAdd=:time where path=:path")
    public abstract void updateTimeRecent(long time, @org.jetbrains.annotations.NotNull()
    java.lang.String path);
    
    /**
     * Inserts a recent item into the database.
     * @param imageDetail the recent item to be inserted.
     * @return the row ID of the newly inserted recent item.
     * @author AnhTTH
     */
    @androidx.room.Insert()
    public abstract long insertRecent(@org.jetbrains.annotations.NotNull()
    com.example.appa.data.model.ItemRecent imageDetail);
    
    /**
     * Updates the download path of a recent item in the database.
     * @param pathDownload the new download path.
     * @param path the original path of the recent item to be updated.
     * @author AnhTTH
     */
    @androidx.room.Query(value = "Update ItemRecent set pathDownload=:pathDownload where path=:path")
    public abstract void updatePathDownload(@org.jetbrains.annotations.NotNull()
    java.lang.String pathDownload, @org.jetbrains.annotations.NotNull()
    java.lang.String path);
}